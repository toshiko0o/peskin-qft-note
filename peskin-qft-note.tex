...
% ams大礼包
\usepackage{amsmath}
\usepackage{amscd} % 制作交换图表 (Commutative diagram)
\usepackage{amssymb}
\usepackage{amsfonts}
...
% \usepackage{zhlipsum}  % 无内容时查看排版效果
% \usepackage{savesym}   % 冰岛语字体包? 
% \usepackage{fancyhdr}  % 调整页眉页脚
% \usepackage{wrapfig}   % 处理小宽度图, 使图位于文本的一边, 并使文本在其边上折行
% \usepackage[all]{xy}   % 制作某种复杂的数学图表
% \usepackage{tikz}      % 某种插入图片的包, 手册1000多页, 不明觉厉
% \usepackage{fancybox}  % 生成花哨的箱子 (比如有阴影的)
...
\chapter*{前言} %开始一段不带编号的章
\addcontentsline{toc}{chapter}{前言} %使目录中以章级别显示
  ...
  \framebox[1.15\width][s]{
    \parbox{32em}{
      \begin{enumerate}
        ...
        　　或者直接将场理解为某种实体在时空中的分布, 它是时空坐标$x^\mu$的函数, 是一个基础的力学量. 
...
\begin{center}
  \framebox[1.15\width][s]{
    \parbox[t]{32em}{
      (\textit{个人理解})只有边界是类空超曲面时, 向空间的无穷远处积分才能到达时间上的边界. 可以画个图感受一下. 
    }
  }
\end{center}
...
\begin{center}
  \framebox[1.15\width][s]{
    \parbox[t]{32em}{
      ...
      在求复数场运动方程时, 往往会将场$\phi$和它的共轭$\phi^*$ (共轭转置$\phi^\dagger$) 当作独立的场处理, 但实际上它们之间存在着约束关系. 可以认为是独立的变量的原因如下 (参见\href{https://arxiv.org/abs/1110.5013v5}{这篇笔记}第56页下方):
      ...
      \begin{enumerate}
        ...
        \begin{equation*}
          \left\{\begin{gathered}
            A + A^* = 0 \\
            A - A^* = 0
          \end{gathered}\right.\Rightarrow A = A^* = 0
        \end{equation*}
      \end{enumerate}
      两种方法得到的结果一致. 
    }
...
\begin{center}
  \framebox[1.15\width][s]{
    \parbox[t]{32em}{
      (\textit{个人理解})可以这样解释的原因是: 在量子力学中我们将两个态的内积的模的平方解释为该态可被观测到的概率, 是可观测量. 而书中前面虽将$\phi(x) |0\rangle$类比为了$|x\rangle$, 但没有赋予其其他含义. (可以理解为这个传播振幅无法与可观测量联系起来, 故无法传递信息). 而算符$\phi(x), \phi(y)$的对易关系与量子力学中算符的对易关系的含义相同. 
    }
  }
\end{center}
...
\begin{center}
  \framebox[1.15\width][s]{
    \parbox[t]{32em}{
      (\textit{评论})一开始学到这里的时候我很迷惑, 不过习惯了就好了. 对这些含有张量的表达式或运算感到疑惑的时候, 把分量具体写出来算一遍总是个好办法. 
    }
  }
\end{center}
...
\begin{center}
  \framebox[1.15\width][s]{
    \parbox[t]{32em}{
      (\textit{评论})注意区分矢量空间和其他表示空间 (基本都是旋量空间) 的指标. 一般后者会被省略. 
    }
  }
\end{center}
...
\begin{center}
  \framebox[1.15\width][s]{
    \parbox[t]{32em}{
      ...
      此展开式也可以由$U(y, x)$更为一般的定义(参考15.3节)得到. 我们令其起点为$0$, 终点为$s$, 则可记$U(s, 0)$为$U(s)$, 且$U(0) = 1$, 其定义式为
      \begin{equation}
...
\clearpage % 加这个命令后, 目录中的页码才显示正确
\phantomsection % 加这个命令后, 目录中的超链接才指向正确的页码
\part*{Epilogue} %开始一段不带编号的章
\addcontentsline{toc}{part}{Epilogue} %使目录中以章级别显示
